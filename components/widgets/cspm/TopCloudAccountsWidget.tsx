        ...
        <div className="glass-panel p-6 rounded-2xl flex flex-col h-full border border-border/50">
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center space-x-2">
                    ...
                    <h3 className="text-lg font-semibold text-foreground tracking-tight">At-Risk Accounts</h3>
                </div>
            </div>

            <div className="flex flex-col space-y-3 flex-1 justify-center">
                {accounts.sort((a, b) => a.score - b.score).map((acc, idx) => (
                    <div key={idx} className="p-3 bg-card/40 rounded-xl border border-border/50 flex flex-col relative overflow-hidden group hover:bg-secondary/40 transition-colors cursor-pointer">
                        <div className="flex justify-between items-center z-10">
                            <div className="flex items-center space-x-2">
                                {acc.score < 70 && <Flame className="w-4 h-4 text-orange-500 animate-pulse" />}
                                <span className="text-sm font-medium text-foreground group-hover:text-blue-400 transition-colors">{acc.name}</span>
                            </div>
                            <span className="text-xs font-mono text-muted-foreground font-bold">{acc.score}/100</span>
                        </div>
                        <div className="flex items-center justify-between mt-1 z-10">
                            ...
                            <span className="text-[10px] text-muted-foreground font-bold uppercase">{acc.provider}</span>
                        </div>
