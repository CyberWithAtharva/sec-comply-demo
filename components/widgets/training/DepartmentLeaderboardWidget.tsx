    ...
    return (
        <div className="glass-panel p-6 rounded-2xl flex flex-col h-full border border-border/50">
            ...
            <div className="flex flex-col space-y-3 mt-2">
                {depts.map((d, i) => (
                    <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-card/40 border border-border/50 hover:bg-secondary/40 transition-colors">
                        <div className="flex items-center space-x-4">
                            <span className={cn(
                                "text-lg font-black w-6 text-center",
                                d.rank === 1 ? "text-amber-400" : d.rank === 2 ? "text-muted-foreground" : d.rank === 3 ? "text-amber-600" : "text-muted-foreground/70"
                            )}>{d.rank}</span>
