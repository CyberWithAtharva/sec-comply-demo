...
const HeatmapCell = ({ cell, category, delay }: { cell: any, category: string, delay: number }) => {
    ...
    let bgClass = "bg-secondary/40 border border-border/30";
            ...
            <AnimatePresence>
                {isHovered && (
                    <motion.div
                        ...
                        className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-56 p-4 rounded-xl bg-card/95 backdrop-blur-md border border-border/80 shadow-2xl z-50 pointer-events-none"
                    >
                        <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-4 h-4 bg-card/95 border-b border-r border-border/80 rotate-45" />
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between items-center text-xs">
                                <span className="text-muted-foreground flex items-center gap-1.5"><ShieldCheck className="w-3.5 h-3.5 text-emerald-500" /> Passed</span>
                                <span className="text-foreground font-semibold">{passedControls}</span>
                            </div>
                            <div className="flex justify-between items-center text-xs">
                                <span className="text-muted-foreground flex items-center gap-1.5"><ShieldAlert className="w-3.5 h-3.5 text-red-500" /> Gaps</span>
                                <span className="text-foreground font-semibold">{failedControls}</span>
                            </div>
                            <div className="w-full h-1.5 bg-secondary rounded-full mt-2 overflow-hidden">
                                <div className="h-full bg-blue-500 rounded-full" style={{ width: `${compliancePercent}%` }} />
                            </div>
                            <div className="text-[10px] text-muted-foreground text-center uppercase tracking-wider mt-2 pt-2 border-t border-border/50">
    ...
    return (
        <div className="glass-panel rounded-2xl p-6 flex flex-col col-span-full xl:col-span-2 h-full min-h-[320px] relative overflow-visible">
            <div className="flex items-center justify-between mb-8">
                <h3 className="text-lg font-semibold text-foreground tracking-tight">Compliance Density Matrix</h3>
                <div className="flex items-center space-x-3 text-xs text-muted-foreground font-medium">
                    <span>Low</span>
                    <div className="flex space-x-1.5">
                        <div className="w-3.5 h-3.5 rounded-[3px] bg-secondary/40 border border-border/30" />
            ...
            <div className="flex-1 w-full flex items-center justify-center overflow-x-auto no-scrollbar pb-6">
                <div className="min-w-[700px] flex flex-col justify-between h-full gap-4">
                    {/* Months Header */}
                    <div className="flex justify-between text-xs text-muted-foreground font-semibold tracking-wider pl-20 pr-4">
                    ...
                    {/* Matrix Body */}
                    <div className="flex flex-col justify-around flex-1 gap-2.5 shrink-0 pt-2 pb-2 border-l border-border/50 pl-4 ml-16 relative">
                            ...
                            return (
                                <div key={category} className="flex items-center gap-3 w-full">
                                    <div className="absolute left-[-4.5rem] flex items-center text-xs w-16 group">
                                        <span className="font-bold text-muted-foreground group-hover:text-blue-400 transition-colors w-4">{letter}</span>
                                        <span className="text-muted-foreground group-hover:text-foreground transition-colors truncate">{category}</span>
                                    </div>
