                ...
                <>
                    ...
                    <div className="glass-panel p-5 rounded-2xl border border-border/50">
                        <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center space-x-2">
                                ...
                                <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Approval Pipeline</span>
                            </div>
                            <Button variant="plain"
                                onClick={handleGenerate}
                                disabled={generating}
                                className="flex items-center gap-1.5 text-xs text-blue-400 hover:text-blue-300 transition-colors disabled:opacity-50 h-auto"
                            >
                                {generating
                                ...
                                <React.Fragment key={stage.stage}>
                                    <div className={cn(
                                        ...
                                        <div>
                                            ...
                                            <span className="text-[10px] text-muted-foreground uppercase tracking-wider block">{stage.stage}</span>
                                        </div>
                                    </div>
                                    {i < 2 && (
                                        <div className="flex items-center flex-shrink-0">
                                            <div className="w-8 h-px bg-secondary" />
                                            <ArrowUpRight className="w-3 h-3 text-muted-foreground/70 -ml-1" />
                                        </div>
                                    )}
                                </React.Fragment>
                            ))}
                    ...
                    <div className="glass-panel p-6 rounded-2xl border border-border/50">
                        <div className="flex justify-between items-center mb-6 border-b border-border/50 pb-4">
                            <h3 className="text-lg font-semibold text-foreground">Governing Standards</h3>
                            <a
                                href="/policies"
                                            ...
                                            <React.Fragment key={pol.id}>
                                                <tr
                                                    ...
                                                    className={cn(
                                                        ...
                                                        isExpanded ? "bg-blue-500/[0.03]" : "hover:bg-secondary/30"
                                                    )}
                                                >
                                                    <td className="px-4 py-3 font-medium text-foreground group-hover:text-blue-400 transition-colors max-w-[280px]">
                                                    ...
                                                    <td className="px-4 py-3">
                                                        <div className="flex items-center justify-center space-x-2">
                                                            <div className="w-16 bg-secondary/60 rounded-full h-1 overflow-hidden">
                                                                <div
                                                                    className={cn(
                                                                        "h-full rounded-full",
                                                            ...
                                                            <span className={cn(
                                                                "text-[10px] font-bold",
                                                                coverage >= 80 ? "text-emerald-400" : coverage >= 50 ? "text-amber-400" : "text-muted-foreground"
                                                            )}>
                                                                {coverage}%
                                                            </span>
                                                        </div>
                                                                        ...
                                                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                                                            ...
                                                                            <div className="bg-card/40 rounded-xl border border-border/50 p-4">
                                                                                <div className="flex items-center space-x-2 mb-3">
                                                                                    ...
                                                                                    <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Policy Details</span>
                                                                                </div>
                                                                                <div className="space-y-2.5 text-xs">
                                                                                    <div className="flex justify-between">
                                                                                        <span className="text-muted-foreground">Version</span>
                                                                                        <span className="font-mono text-muted-foreground">{pol.version || "1.0"}</span>
                                                                                    </div>
                                                                                    <div className="flex justify-between">
                                                                                        ...
                                                                                        <span className={cn(
                                                                                            ...
                                                                                            pol.status === "approved" ? "text-emerald-400" :
                                                                                                pol.status === "under_review" ? "text-amber-400" : "text-muted-foreground"
                                                                                        )}>{displayStatus}</span>
                                                                                    </div>
                                                                                    <div className="flex justify-between">
                                                                                        <span className="text-muted-foreground">Next Review</span>
                                                                                        <span className="text-muted-foreground">{formatDate(pol.next_review)}</span>
                                                                                    </div>
                                                                                    <div className="flex justify-between">
                                                                                        <span className="text-muted-foreground">Last Updated</span>
                                                                                        <span className="text-muted-foreground">{formatRelative(pol.updated_at)}</span>
                                                                                    </div>
                                                                                </div>
                                                                            </div>

                                                                            {/* Action card */}
                                                                            <div className="md:col-span-2 bg-card/40 rounded-xl border border-border/50 p-4 flex flex-col items-center justify-center space-y-3">
                    ...
                    {policies.some(p => p.next_review) && (
                        <div className="glass-panel p-6 rounded-2xl border border-border/50">
                            <div className="flex items-center space-x-2 mb-4">
                                ...
                                <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider">Upcoming Reviews</h3>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                                {policies
                                    ...
                                    .map((pol) => (
                                        <div
                                            key={pol.id}
                                            className="flex items-center space-x-3 p-3 bg-card/40 rounded-xl border border-border/50 hover:bg-secondary/40 transition-colors"
                                        >
                                            <div className={cn(
                                            ...
                                            <div className="flex flex-col min-w-0">
                                                <span className="text-xs font-medium text-foreground truncate">{pol.title}</span>
                                                <span className="text-[10px] text-muted-foreground">{formatDate(pol.next_review)}</span>
                                            </div>
                                        </div>
                                    ))
                                }
