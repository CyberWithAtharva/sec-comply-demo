                ...
                <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            />
                            <h1 className="text-2xl font-bold text-foreground truncate">{framework.name}</h1>
                            {framework.version && (
                                <span className="text-sm text-muted-foreground">v{framework.version}</span>
                            )}
                        </div>
                        {framework.description && (
                            <p className="text-sm text-muted-foreground max-w-3xl">{framework.description}</p>
                        )}
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                        <Button variant="plain"
                            ...
                            className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-border hover:bg-secondary text-xs text-muted-foreground transition-colors h-auto"
                        >
            ...
            <div className="flex items-center gap-3">
                <div className="relative flex-1 max-w-md">
                    ...
                    <input
                        ...
                        className="w-full bg-background/50 border border-border rounded-xl pl-9 pr-4 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-amber-500/40"
                    />
                </div>
                <Button variant="plain"
                    type="button"
                    onClick={() => setShowBulkDialog(true)}
                    className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-border hover:bg-secondary text-sm text-muted-foreground transition-colors h-auto"
                >
                ...
                <Button variant="plain"
                    type="button"
                    onClick={openCreate}
                    className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-amber-500 hover:bg-amber-400 text-black font-semibold text-sm transition-colors h-auto"
                >
            ...
            <div className="rounded-2xl border border-border/60 bg-card/30 backdrop-blur-sm overflow-hidden">
                {total === 0 ? (
                    <div className="p-16 text-center">
                        ...
                        <p className="text-muted-foreground font-medium">No controls in this framework yet</p>
                        <p className="text-sm text-muted-foreground mt-1">Add controls one by one or upload a CSV/XLSX in bulk.</p>
                    </div>
                ) : (
                                    ...
                                    <td className="px-4 py-2.5 text-right">
                                        <div className="opacity-0 group-hover:opacity-100 transition-opacity inline-flex gap-1">
                                            <Button variant="plain"
                                                type="button"
                                                onClick={() => openEdit(c)}
                                                className="p-1.5 rounded-md hover:bg-secondary text-muted-foreground hover:text-foreground transition-colors h-auto"
                                                title="Edit"
                                            >
                                            ...
                                            <Button variant="plain"
                                                ...
                                                className="p-1.5 rounded-md hover:bg-red-950/60 text-muted-foreground hover:text-red-400 transition-colors disabled:opacity-40 h-auto"
                                                title="Delete"
                                            >
