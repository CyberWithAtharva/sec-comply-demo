...
export function RiskPostureSummary({ risks, history, nowMs }: Props) {
    ...
    const trendColor = trend === "improving" ? "text-emerald-400" : trend === "worsening" ? "text-rose-400" : "text-slate-400";
        ...
        <div className="space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {cards.map(c => (
                    <div key={c.label} className={`glass-panel p-5 rounded-2xl ${c.bg}`}>
                        <p className="text-xs font-medium text-slate-500 uppercase tracking-wider mb-2">{c.label}</p>
                        <p className={`text-3xl font-bold tracking-tight ${c.color}`}>{c.value}</p>
                        <p className="text-[10px] text-slate-600 uppercase tracking-wider mt-1">active</p>
                    </div>
                ))}
            </div>
            ...
            <div className="glass-panel rounded-2xl p-5">
                <div className="flex items-center justify-between mb-3">
                    <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider">Posture by Status</h3>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
                    ...
                    <Stat label="Closed"        value={closed} hue="text-slate-400" />
                </div>
            </div>
        </div>
    );
...
function Stat({ label, value, hue }: { label: string; value: number; hue: string }) {
    return (
        <div className="bg-slate-950/40 border border-slate-800/40 rounded-xl px-3 py-2.5">
