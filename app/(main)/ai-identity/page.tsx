...
const SERVICE_COLOR: Record<string, string> = {
    ...
    GitHub: "bg-slate-600/50 text-muted-foreground",
        ...
        <div className="w-full flex flex-col space-y-6 animate-in fade-in duration-700">
            ...
            <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-4">
                    ...
                    <div>
                        <div className="flex items-center gap-3">
                            <h1 className="text-2xl font-bold text-foreground tracking-tight">AI Identity</h1>
                ...
                <Button variant="plain"
                    type="button"
                    className="flex items-center gap-2 px-4 py-2 bg-blue-600/20 hover:bg-blue-600/30 border border-blue-500/30 text-blue-300 text-sm font-medium rounded-xl transition-colors shrink-0 h-auto"
                    onClick={() => {}}
                >
                ...
                <div className="col-span-2 bg-card/60 border border-border/60 rounded-2xl overflow-hidden">
                    <div className="flex items-center justify-between px-5 py-4 border-b border-border/60">
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-sm font-semibold text-foreground">Identity Registry</h2>
                        </div>
                        <span className="text-xs text-muted-foreground">{AI_IDENTITIES.length} identities</span>
                    </div>
                                ...
                                {AI_IDENTITIES.map((id, i) => (
                                    <tr
                                        ...
                                        className={cn(
                                            ...
                                            selectedId === id.id ? "bg-blue-500/5 border-blue-500/20" : "hover:bg-secondary/30"
                                        )}
                                    >
                                        <td className="px-4 py-3 font-mono text-muted-foreground">{id.id}</td>
                                        <td className="px-4 py-3 font-mono text-xs text-muted-foreground whitespace-nowrap">{id.name}</td>
                                        <td className="px-4 py-3">
                                            <span className={cn("text-[10px] font-semibold px-2 py-0.5 rounded-full border whitespace-nowrap", TYPE_STYLE[id.type] ?? "text-muted-foreground bg-slate-500/10 border-slate-500/20")}>
                                                {id.type}
                                            </span>
                                        </td>
                                        <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">{id.owner}</td>
                                        <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">{id.service}</td>
                                        <td className="px-4 py-3 font-mono text-muted-foreground max-w-[140px] truncate">{id.permissions}</td>
                                        <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">{id.lastUsed}</td>
                                        <td className="px-4 py-3">
                        ...
                        return (
                            <div className="px-5 py-4 border-t border-blue-500/20 bg-blue-500/5">
                                <div className="flex items-start gap-6">
                                    <div>
                                        <p className="text-[11px] text-muted-foreground font-semibold uppercase tracking-wider mb-1">Expires</p>
                                        <p className="text-sm font-mono text-muted-foreground">{id.expires}</p>
                                    </div>
                                    <div>
                                        <p className="text-[11px] text-muted-foreground font-semibold uppercase tracking-wider mb-1">Permissions</p>
                                        <p className="text-sm font-mono text-muted-foreground">{id.permissions}</p>
                                    </div>
                                    <div className="ml-auto flex items-center gap-2">
                                        <Button variant="plain" className="flex items-center gap-1.5 px-3 py-1.5 bg-secondary border border-border rounded-lg text-xs text-muted-foreground hover:bg-secondary transition-colors h-auto">
                                            <RotateCw className="w-3 h-3" /> Rotate
                                        </Button>
                                        <Button variant="plain" className="flex items-center gap-1.5 px-3 py-1.5 bg-secondary border border-border rounded-lg text-xs text-muted-foreground hover:bg-secondary transition-colors h-auto">
                                            <Eye className="w-3 h-3" /> Audit Log
                                        </Button>
                                        <Button variant="plain" className="flex items-center gap-1.5 px-3 py-1.5 bg-red-500/10 border border-red-500/20 rounded-lg text-xs text-red-400 hover:bg-red-500/20 transition-colors h-auto">
                                ...
                                return (
                                    <div key={p.check}>
                                        <div className="flex items-center justify-between mb-1">
                                            <span className="text-[11px] text-muted-foreground leading-snug flex-1 pr-2">{p.check}</span>
                ...
                <div className="divide-y divide-border/40">
                    {ACTIVITY_LOG.map((entry, i) => (
                        <div key={i} className={cn(
                            ...
                            entry.alert ? "bg-red-500/5 hover:bg-red-500/8" : "hover:bg-secondary/20"
                        )}>
                            <span className="text-xs text-muted-foreground/70 w-24 shrink-0">{entry.time}</span>
                            <span className={cn("text-[11px] font-mono px-2 py-0.5 rounded shrink-0", SERVICE_COLOR[entry.service] ?? "bg-secondary text-muted-foreground")}>
                                {entry.service}
                            </span>
                            <span className="text-xs font-mono text-muted-foreground shrink-0">{entry.identity}</span>
                            <span className="text-xs text-muted-foreground flex-1 truncate">{entry.event}</span>
                            {entry.alert && (
