...
const STATUS_CONFIG: Record<LogStatus, { cls: string; icon: React.ReactNode }> = {
    ...
    "False Positive": { cls: "bg-slate-500/15 text-muted-foreground border-slate-500/30", icon: <X className="w-3 h-3" /> },
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <motion.div
                ...
                className="bg-card border border-border/50 rounded-2xl w-full max-w-2xl shadow-2xl max-h-[90vh] overflow-y-auto"
            >
                <div className="flex items-start justify-between p-5 border-b border-border/50">
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <span className="text-sm font-bold text-foreground font-mono">{log.id}</span>
                        ...
                        <p className="text-xs text-muted-foreground">{log.system} · {log.timestamp} · {log.user}</p>
                    </div>
                    <Button variant="plain" onClick={onClose} className="text-muted-foreground hover:text-muted-foreground transition-colors mt-0.5 h-auto"><X className="w-5 h-5" /></Button>
                </div>

                <div className="p-5 space-y-4">
                    <div className="space-y-3">
                        <div className="bg-secondary/50 border border-border/40 rounded-xl p-4">
                            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-2">Input Prompt</p>
                            <p className="text-sm text-muted-foreground leading-relaxed">{log.prompt}</p>
                        </div>
                        <div className="bg-secondary/50 border border-border/40 rounded-xl p-4">
                            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-2">AI Output</p>
                            <p className="text-sm text-muted-foreground leading-relaxed">{log.output}</p>
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 text-xs">
                        ...
                        <div className="bg-secondary/30 rounded-xl p-3">
                            <p className="text-muted-foreground mb-1">Assigned Reviewer</p>
                            <p className="text-muted-foreground font-medium">compliance.team</p>
                        </div>
                    </div>

                    <div>
                        <label className="block text-xs font-medium text-muted-foreground mb-1.5">Resolution Notes</label>
                        <textarea
                            value={notes} onChange={e => setNotes(e.target.value)} rows={3}
                            placeholder="Add resolution notes or escalation reason…"
                            className="w-full bg-secondary/60 border border-border/50 rounded-xl px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground/70 focus:outline-none focus:border-orange-500/50 transition-colors resize-none"
                        />
                    </div>

                    <div className="flex justify-end gap-3">
                        <Button variant="plain" onClick={onClose} className="px-4 py-2 text-sm text-muted-foreground hover:text-foreground transition-colors h-auto">Cancel</Button>
                        <Button variant="plain" onClick={onClose} className="px-4 py-2 bg-emerald-700 hover:bg-emerald-600 text-white text-sm font-medium rounded-xl flex items-center gap-2 transition-colors h-auto">
        ...
        <div className="w-full flex flex-col space-y-5 animate-in fade-in duration-700">
            ...
            <div className="flex items-start justify-between gap-4">
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <h1 className="text-2xl font-bold text-foreground tracking-tight">AI Output Monitoring</h1>
                    ...
                    <p className="text-sm text-muted-foreground">Monitor AI system outputs for policy violations, hallucinations, and harmful content</p>
                </div>
            </div>
            ...
            <div className="bg-card/60 border border-border/60 rounded-xl p-5">
                <div className="flex items-center justify-between mb-4">
                    <div>
                        <h2 className="text-sm font-semibold text-foreground">AI Output Volume & Violations</h2>
                        <p className="text-xs text-muted-foreground mt-0.5">Last 30 days</p>
                    </div>
                    <div className="flex items-center gap-4 text-xs">
                        <div className="flex items-center gap-1.5"><div className="w-3 h-0.5 bg-indigo-400 rounded" /><span className="text-muted-foreground">Total Outputs</span></div>
                        <div className="flex items-center gap-1.5"><div className="w-3 h-0.5 bg-orange-400 rounded" /><span className="text-muted-foreground">Flagged</span></div>
                    </div>
                </div>
            ...
            <div className="bg-card/60 border border-border/60 rounded-xl p-4">
                <div className="flex flex-wrap gap-3">
                    <div className="relative flex-1 min-w-48">
                        <Activity className="absolute left-3 top-2.5 w-4 h-4 text-muted-foreground pointer-events-none" />
                        <input
                    ...
                    {(search || filterSeverity !== "all" || filterStatus !== "all" || filterViolation !== "all") && (
                        <Button variant="plain" onClick={() => { setSearch(""); setFilterSeverity("all"); setFilterStatus("all"); setFilterViolation("all"); }}
            ...
            <div className="bg-card/60 border border-border/60 rounded-xl overflow-hidden">
                <div className="px-5 py-3 border-b border-border/60">
                    <h2 className="text-sm font-semibold text-foreground">Flagged Output Log</h2>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                                ...
                                return (
                                    <tr key={l.id} className="border-b border-border/40 hover:bg-secondary/20 transition-colors">
                                        <td className="px-4 py-3">
                                            <span className="font-mono text-xs font-semibold text-muted-foreground">{l.id}</span>
                                        </td>
                                        <td className="px-4 py-3 text-xs text-muted-foreground whitespace-nowrap max-w-[160px] truncate">{l.system}</td>
                                        <td className="px-4 py-3 text-xs text-muted-foreground whitespace-nowrap">{l.timestamp}</td>
                                        <td className="px-4 py-3 text-xs text-muted-foreground font-mono whitespace-nowrap">{l.user}</td>
                                        <td className="px-4 py-3">
                                        ...
                                        <td className="px-4 py-3">
                                            <div className="flex items-center gap-1">
                                                <Button variant="plain" onClick={() => setSelectedLog(l)}
                                                    className="px-2 py-1 text-[10px] font-semibold bg-blue-500/10 hover:bg-blue-500/20 text-blue-300 border border-blue-500/25 rounded-lg transition-colors whitespace-nowrap flex items-center gap-1 h-auto">
                                                    <Eye className="w-3 h-3" /> View
                                                </Button>
                                                <Button variant="plain" className="px-2 py-1 text-[10px] font-semibold bg-purple-500/10 hover:bg-purple-500/20 text-purple-300 border border-purple-500/25 rounded-lg transition-colors whitespace-nowrap flex items-center gap-1 h-auto">
                                                    <ArrowUpRight className="w-3 h-3" /> Escalate
                                                </Button>
                                                <Button variant="plain" className="px-2 py-1 text-[10px] font-semibold bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-300 border border-emerald-500/25 rounded-lg transition-colors whitespace-nowrap flex items-center gap-1 h-auto">
                            ...
                            {filtered.length === 0 && (
                                <tr>
                                    <td colSpan={8} className="px-4 py-12 text-center text-muted-foreground text-sm">No logs match your filters.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
                ...
                <div className="px-4 py-3 border-t border-border/40">
                    <span className="text-xs text-muted-foreground">{filtered.length} of {logs.length} flagged logs</span>
                </div>
            </div>

            {/* Monitoring Policies */}
            <div className="bg-card/60 border border-border/60 rounded-xl p-5">
                <h2 className="text-sm font-semibold text-foreground mb-1">Monitoring Policies</h2>
                <p className="text-xs text-muted-foreground mb-5">Configure detection thresholds and notification rules</p>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                    ...
                    <div className="bg-secondary/30 rounded-xl p-4 border border-border/30">
                        <div className="flex items-center justify-between mb-3">
                            <div>
                                <p className="text-sm font-medium text-foreground">Hallucination Detection Sensitivity</p>
                                <p className="text-xs text-muted-foreground mt-0.5">Higher sensitivity may increase false positives</p>
                            </div>
                        ...
                        <div className="flex gap-2">
                            {["Low", "Medium", "High"].map(s => (
                                <Button variant="plain" key={s} onClick={() => setSensitivity(s)}
                                    className={cn("h-auto", "flex-1 py-1.5 text-xs font-semibold rounded-lg border transition-all",
                                        sensitivity === s
                                            ? "bg-orange-500/20 text-orange-300 border-orange-500/40"
                                            : "bg-secondary/30 text-muted-foreground border-border/40 hover:border-slate-600"
                                    )}>{s}</Button>
                            ))}
                        </div>
                    </div>
                    ...
                    {/* Log retention */}
                    <div className="bg-secondary/30 rounded-xl p-4 border border-border/30">
                        <p className="text-sm font-medium text-foreground mb-1">Retain Output Logs For</p>
                        <p className="text-xs text-muted-foreground mb-3">Logs older than this period are purged automatically</p>
                        <div className="relative">
                            <select value={retention} onChange={e => setRetention(e.target.value)}
                                className="w-full bg-secondary/60 border border-border/50 rounded-xl px-3 py-2 text-sm text-muted-foreground focus:outline-none focus:border-orange-500/40 transition-colors cursor-pointer appearance-none">
                    ...
                    {[
                        ...
                        <div key={label} className="bg-secondary/30 rounded-xl p-4 border border-border/30 flex items-start justify-between gap-4">
                            <div>
                                <p className="text-sm font-medium text-foreground">{label}</p>
                                <p className="text-xs text-muted-foreground mt-0.5">{desc}</p>
                            </div>
                            <Button variant="plain" onClick={() => set(v => !v)}
                                className={cn("h-auto", "relative inline-flex h-6 w-11 shrink-0 items-center rounded-full border-2 transition-colors duration-200 focus:outline-none",
                                    val ? "bg-orange-500 border-orange-500" : "bg-secondary border-slate-600")}>
